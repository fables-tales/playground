...
describe "CallOriginWithModule" do
  ...
  it "calls the generate once" do
    p (class << JrJackson::Raw; self; end).instance_method(:generate).owner
    expect(JrJackson::Raw).to receive(:generate).with({:source=>3, :target=>-3}).and_call_original
    p (class << JrJackson::Raw; self; end).instance_method(:generate).owner
    p (class << JrJackson::Raw; self; end).instance_method(:generate).owner == (class << JrJackson::Raw; self; end)
    expect(AClass.bar(-3)).to eq("{\"source\":3,\"target\":-3}")
  end

  it "calls the generate twice" do
    (class << JrJackson::Raw; self; end).instance_method(:generate).owner
    expect(JrJackson::Raw).to receive(:generate).with({:source=>3, :target=>3}).and_call_original
    expect(AClass.bar(3)).to eq("{\"source\":3,\"target\":3}")
  end
